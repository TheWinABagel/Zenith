...
public class PotionAffix extends Affix {
	...
	public static final Codec<PotionAffix> CODEC = RecordCodecBuilder.create(inst -> inst
		.group(
			Registry.MOB_EFFECT.byNameCodec().fieldOf("mob_effect").forGetter(a -> a.effect),
	...
	private void applyEffect(LivingEntity target, EffectInst inst, float level) {
		...
		if (this.cooldown != 0) {
			long lastApplied = target.getExtraCustomData().getLong("apoth.affix_cooldown." + this.getId().toString());
			if (lastApplied != 0 && lastApplied + this.cooldown >= target.level.getGameTime()) return;
		}
		target.addEffect(mei);
		target.getExtraCustomData().putLong("apoth.affix_cooldown." + this.getId().toString(), target.level.getGameTime());
	}
