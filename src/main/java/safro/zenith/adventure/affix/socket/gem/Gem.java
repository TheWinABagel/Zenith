...
import safro.zenith.adventure.loot.LootRarity.Clamped;
import safro.zenith.api.placebo.codec.PlaceboCodecs;
...
import safro.zenith.api.placebo.json.WeightedJsonReloadListener.ILuckyWeighted;
...
public class Gem extends TypeKeyedBase<Gem> implements ILuckyWeighted, IDimensional, Clamped {
	...
	public static final Codec<Gem> CODEC = RecordCodecBuilder.create(inst -> 
		inst.group(
			...
			LootRarity.CODEC.optionalFieldOf("min_rarity", LootRarity.COMMON).forGetter(Clamped::getMinRarity),
			LootRarity.CODEC.optionalFieldOf("max_rarity", LootRarity.MYTHIC).forGetter(Clamped::getMaxRarity),
			...
			PlaceboCodecs.setCodec(Codec.STRING).optionalFieldOf("stages").forGetter(gem -> Optional.ofNullable(null)))
			.apply(inst, Gem::new)
		);
	//Formatter::on
