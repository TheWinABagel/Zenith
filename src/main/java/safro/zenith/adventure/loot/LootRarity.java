package safro.zenith.adventure.loot;
...
import safro.zenith.api.RainbowColor;
...
public class LootRarity implements ILuckyWeighted, Comparable<LootRarity> {
	...
	public static final LootRarity ANCIENT = new LootRarity("ancient", RainbowColor.RAINBOW, 5, 0, 0, ImmutableList.of(
	...
	public static class RarityStub extends TypeKey.TypeKeyedBase<RarityStub> implements ILuckyWeighted {
		//Formatter::off
		...
		public void execute(ItemStack stack, LootRarity rarity, Set<Affix> currentAffixes, MutableInt sockets, RandomSource rand) {
			...
			if (rand.nextFloat() <= this.chance) {
				...
				if (available.size() == 0) {
					...
					else AdventureModule.LOGGER.error("Failed to execute LootRule {}/{}/{}/{}!", Registry.ITEM.getKey(stack.getItem()), rarity.id(), this.type, this.chance);
					return;
				}
