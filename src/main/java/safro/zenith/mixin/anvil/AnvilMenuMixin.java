...
@Mixin(value = AnvilMenu.class, priority = 999)
    ...
    @Redirect(method = "createResult", at = @At(value = "INVOKE", target = "net/minecraft/world/item/enchantment/Enchantment.canEnchant (Lnet/minecraft/world/item/ItemStack;)Z"))
    private boolean zenithCanEnchant(Enchantment instance, ItemStack itemStack){
        if (Zenith.enableEnch){
            if (Zenith.DEBUG) Zenith.LOGGER.warn("CANENCH: canApply {}, and canEnch: {}", (ZenithUtil.canApply(instance, itemStack)), instance.canEnchant(itemStack));
            return ZenithUtil.canApply(instance, itemStack) || instance.canEnchant(itemStack);
        }
        return instance.canEnchant(itemStack);
    }
