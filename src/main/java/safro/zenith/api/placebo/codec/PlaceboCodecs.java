package safro.zenith.api.placebo.codec;
...
public class PlaceboCodecs {
	...
	public static class MapBackedCodec<V extends CodecProvider<V>> implements Codec<V> {
		...
		public <T> DataResult<Pair<V, T>> decode(DynamicOps<T> ops, T input) {
			...
			Optional<ResourceLocation> key = type.map(t -> ResourceLocation.CODEC.decode(ops, t).resultOrPartial(Zenith.LOGGER::error).get().getFirst());
		...
		public <T> DataResult<T> encode(V input, DynamicOps<T> ops, T prefix) {
			...
			T encodedKey = ResourceLocation.CODEC.encodeStart(ops, key).getOrThrow(false, Zenith.LOGGER::error);
			T encodedObj = (T) codec.encode(input, ops, prefix).getOrThrow(false, Zenith.LOGGER::error);
