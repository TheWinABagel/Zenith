...
import safro.apotheosis.api.Expression;
...
public class EnchantmentInfo {
    ...
    public static EnchantmentInfo load(Enchantment ench, Configuration cfg) {
        ResourceLocation name = Registry.ENCHANTMENT.getKey(ench);
        ...
        String rarity = cfg.getString("Rarity", name.toString(), ench.getRarity().name(), "The rarity of this enchantment.  Valid values are COMMON, UNCOMMON, RARE, and VERY_RARE.");
        try {
            Enchantment.Rarity r = Enchantment.Rarity.valueOf(rarity);
            ench.rarity = r;
        } catch (Exception ex) {
            EnchModule.LOGGER.error("Failed to parse rarity for {}, as {} is not a valid rarity string.", name, rarity);
        }
        return info;
    }
