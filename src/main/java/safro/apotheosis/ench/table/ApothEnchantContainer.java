...
import safro.apotheosis.util.FloatReferenceHolder;
...
public class ApothEnchantContainer extends EnchantmentMenu {
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        this.access.execute((world, pos) -> {
            ...
            if (!list.isEmpty()) {
                ...
                if (list.get(0).enchantment == EnchModule.INFUSION) {
                    ...
                    for (int slot = 0; slot < 3; ++slot) {
                        if (this.costs[slot] > 0) {
                            ...
                            if (list != null && !list.isEmpty()) {
                                ...
                                ClueMessage.sendTo(this.player, slot, clueList, list.isEmpty());
                            }
                        }
                    }
    ...
    private List<EnchantmentInstance> getEnchantmentList(ItemStack stack, int enchantSlot, int level) {
        ...
        if (enchantSlot == 2 && match != null) {
            list.clear();
            list.add(new EnchantmentInstance(EnchModule.INFUSION, 1));
        }
        return list;
    }
    ...
    public void gatherStats() {
        this.access.evaluate((world, pos) -> {
            ...
            this.arcana.set(stats[2] + this.getSlot(0).getItem().getItem().getEnchantmentValue() / 2F);
    ...
    public MenuType<?> getType() {
        return EnchModule.ENCHANTING_TABLE_MENU;
    }
