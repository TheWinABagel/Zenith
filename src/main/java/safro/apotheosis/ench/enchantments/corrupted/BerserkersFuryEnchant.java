...
import safro.apotheosis.ench.CorruptedDamageSource;
...
public class BerserkersFuryEnchant extends Enchantment {
	 ...
	 */
	public void doPostHurt(LivingEntity user, Entity entity, int i) {
		if (user.getEffect(MobEffects.DAMAGE_RESISTANCE) == null) {
			int level = EnchantmentHelper.getEnchantmentLevel(this, user);
			if (level > 0) {
				user.invulnerableTime = 0;
				user.hurt(CorruptedDamageSource.DEFAULT, level * level);
