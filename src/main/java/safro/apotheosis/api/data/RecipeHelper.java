package safro.apotheosis.api.data;
...
public class RecipeHelper {
    ...
    public static void addRecipe(Recipe<?> rec) {
        synchronized (recipes) {
            if (rec == null) {
                Apotheosis.LOGGER.error("Attempted to add null recipe, this is invalid behavior.");
                Thread.dumpStack();
            }
            recipes.add(rec);
        }
    ...
    public static NonNullList<Ingredient> createInput(boolean allowEmpty, Object... input) {
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            else throw new UnsupportedOperationException("Attempted to add invalid recipe.  Complain to the author of " + Apotheosis.MODID + ". (Input " + k + " not allowed.)");
        }
        return inputL;
    }
    ...
    static void addRecipes(RecipeManager mgr) {
        recipes.forEach(r -> {
            ...
            if (old == null) {
                map.put(r.getId(), r);
            } else Apotheosis.LOGGER.debug("Skipping registration for code recipe {} as a json recipe already exists with that ID.", r.getId());
        });
        Apotheosis.LOGGER.info("Registered {} additional recipes.", recipes.size());
    }
