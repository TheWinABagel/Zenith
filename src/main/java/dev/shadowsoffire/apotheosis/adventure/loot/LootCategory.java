...
public final class LootCategory {
    ...
    public static final LootCategory SWORD = register("sword",
        s -> s.canPerformAction(ToolActions.SWORD_DIG) || s.getItem().getAttributeModifiers(s, EquipmentSlot.MAINHAND).get(Attributes.ATTACK_DAMAGE).stream().anyMatch(m -> m.getAmount() > 0), arr(EquipmentSlot.MAINHAND));
     ...
     * @param slots A function that provides the loot categories that bonuses will be active for, if an item is of this category.
    ...
    private static class ShieldBreakerTest implements Predicate<ItemStack> {
        ...
        public boolean test(ItemStack t) {
            try {
                ...
                if (t.getItem() instanceof ShieldBlockItem sb || t.getItem() instanceof AxeItem) {
                    if (t.getItem() instanceof ShieldBlockItem) return ((ShieldBlockItem) t.getItem()).canDisableShield(t, shield, this.holder, this.attacker);
                    return true;
                }
