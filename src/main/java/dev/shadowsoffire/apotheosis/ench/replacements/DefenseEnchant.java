...
public class DefenseEnchant extends ProtectionEnchantment {
    ...
    public int getDamageProtection(int level, DamageSource source) {
        ...
        else if (this.type == Type.FIRE && source.is(DamageTypeTags.IS_FIRE)) {
            return level;
        }
        else if (this.type == Type.FALL && source.is(DamageTypeTags.IS_FALL)) {
            return level * 3;
        }
        else if (this.type == Type.EXPLOSION && source.is(DamageTypeTags.IS_EXPLOSION)) {
            return level * 2;
        }
        else {
            return this.type == Type.PROJECTILE && source.is(DamageTypeTags.IS_PROJECTILE) ? level : 0;
        }
    }
